...
public class Graphe {
    ...
    public void replaceCase(Case caseBase, Case newCase) {
        Vertex v = this.getVertex(caseBase);
        v.setCase(newCase);
        this.Vertices.remove(caseBase);
        this.Vertices.put(newCase, v);
        for (Vertex d : this.getVertices().values()) {
            VertexCouple vC = new VertexCouple(d, v);
            if (this.Labels.get(vC) != null) {
                //  this.setLabel(newCase,, 1);
